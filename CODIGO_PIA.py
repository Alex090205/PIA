        ...
        try: 
            ...
            if Nombre == '':
                ...
                if Fecha_dt < fecha_permitida:
                    print("Debes hacer la reservación con 2 dias de anticipación.")
                else:
                    ...
                    try:
                        ...
                        if (conn):
                            ...
                            print("¡Reservación Realizada con exito!")
                            try:
                                with sqlite3.connect("CoworkingLTI", detect_types = sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES) as conn:
                                    ...
                                    for clave, nombre, fecha_registro, fecha in registros:
                                        ...
                                        print(f"{clave}\t {nombre}\t {fecha_registro}'t {fecha}'t")
                                        return
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                print(f"Surgio una falla siendo este la causa: {sys.exc_info()[0]}")
...
def consulta_fecha():
    ...
    fechya_dt = datetime.strptime(fecha_consultar, '%d/%m%Y')

    try:
        with sqlite3.connect("CoworkingLTI", detect_types = sqlite3.PARSE_DECLYPTES | sqlite3.PARSE_COLUMNES) as conn:
            mi_cursor = conn.cursor()
            criterios = {"fecha":fecha_consultar}
            mi_cursor.execute("SELECT folio, nombre,horario, fecha FROM Reservaciones WHERE DATE (fecha) = :fecha;", criterios)
            registros = mi_cursor.fetchall()
        ...
        try:
            with sqlite3.connect("CoworkingLTI", detect_types =sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES) as conn:
                ...
                if registrados:
                    ...
                    print(f"No se encontro un proyecto asociado con la clave {fecha_reporte}")
                    return
        except Error as e:
            print (e)
        except:
...
def Registrar_Cliente ():
    while True:
        Usuario=input("Ingresa al usuario.(Escribe SALIR si quieres regresar al menu principal):")
        n = rd.randint(1,99)
        if Usuario ==" ":
            break
        elif Usuario=="SALIR":
        ...
        try:
            with sqlite3.connect("CoworkingLTI") as conn:
                ...
                if registro:
                    ...
                    print(f"No se encontro un proyecto asociado con la clave {key_code}")
        except Error as e:
            print (e)
        except:
            print(f"Se produjo el siguiente error: {sys.exc_info()[0]}")
            ...
            if id_sala ==" ":
                ...
                fecha_asignada = input("Ingresa la fecha asignada por favor: ")
                fecha_dt = datetime.strptime(fecha_asignada,  '%d/%m%/Y')
                fecha_permitida = datetime.now() + timedelta( days = 3)
                if fecha_dt < fecha_permitida:
                    print("Lo siento pero no la puedes cancelar, debes cancelarla con 3 dias de anticipacion")
                else:
                    try:
                        with sqlite3.connect("CoworkingLTI") as conn:
                            mi_cursor = conn.cursor()
...
def Mi_menu ():
    ...
    Crear_tabla ()
    print("*" * 20)
    print("1. Registra la reservacvion\n"
    ...
    while True:
        try:
            ...
            print(f"Ocurrio un prolema {sys.exc_info()[0]}")
        else:
            if Opcion == 1:
                ...
                print("Eso no esta disponible checa el menu")
